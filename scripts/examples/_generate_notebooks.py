...
from notebook_support import LOSS_WEIGHTS_HELP  # noqa: E402
...
brace geometry and displacement drive.

In this notebook you will:
- pick a base SteelMPF vector (sensible defaults — not all `a1`–`a4` equal to 1)
- choose a drive: synthetic growing cycles, or a real specimen history
...
parameter (base value is always plotted too). `DRIVE` is `"synthetic"` or a
specimen `Name` with resampled data."""
            ),
            cell_code(
...
DRIVE = "synthetic"  # "synthetic" or a Name with data/resampled/{{Name}}/...

# Growing-cycle synthetic protocol [in] (ignored when DRIVE is a specimen Name)
...
print("drive:", DRIVE)"""
            ...
            cell_md(
                """## Build drive + base response

Uses catalog geometry for `SPECIMEN_GEOMETRY`. Synthetic drive is fast and easy to
...
if str(DRIVE).strip().lower() == "synthetic":
    disp = make_cyclic_drive(SYNTHETIC_AMPS, n_quarter=SYNTHETIC_N_QUARTER)
    drive_label = "synthetic"
else:
    disp, _exp_force = load_experiment_force_disp(str(DRIVE))
    drive_label = str(DRIVE)

print(f"drive={drive_label!r}  n={len(disp)}")
...
plt.title(f"Base SteelMPF  ({drive_label}, geometry {SPECIMEN_GEOMETRY})")
...
`SPECIMENS` is the list to fit. `SET_ID_ROW` is one shared seed / loss / free-parameter
row (same idea as `config/calibration/set_id_settings.csv`)."""
            ),
            cell_code(
                f"""from pathlib import Path
import sys
